...
public class TelaVilagemNZ implements Screen {
    ...
    public TelaVilagemNZ(GlobeDelegates jogo, Jogador jogador, boolean escapeCompleto, boolean delegacaoCompleta) {
        this.jogo = jogo;
        ...
        fundo = new Texture("nz/telaEscolha.jpg");
        ...
        for (int i = 0; i < 3; i++) {
            ...
            if (dist < 80 && desbloqueado[i]) {
                ...
                if (Gdx.input.isKeyJustPressed(Keys.SPACE)) {
                    if (i == 0) jogo.setScreen(new TelaEscapeNZ(jogo, jogador));
                    if (i == 1) jogo.setScreen(new TelaDelegacaoNZ(jogo, jogador));
                    if (i == 2) jogo.setScreen(new TelaBonusNZ(jogo, jogador));
                }
            }
        }
