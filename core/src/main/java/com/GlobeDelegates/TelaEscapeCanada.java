...
public class TelaEscapeCanada implements Screen {
    ...
    private String[] nomesArquivo = {"folha", "totem", "castor", "iglu", "xarope", "hoquei"};
    private String[] nomesCorretos = {"Folha de Bordo", "Totem", "Castor", "Iglu", "Xarope de Bordo", "Hoquei"};
    private String[] iniciais = {"FB", "T", "C", "I", "XB", "H"};
    ...
    private float[] objX = {120, 450, 680, 900, 200, 750};
    private float[] objY;

    public TelaEscapeCanada(GlobeDelegates jogo, Jogador jogador) {
        this.jogo = jogo;
        ...
        fundo = new Texture("canada/escape.jpg");

        objetos = new Texture[nomesArquivo.length];
        for (int i = 0; i < nomesArquivo.length; i++) {
            objetos[i] = new Texture("canada/" + nomesArquivo[i] + ".png");
        }
    ...
    private void renderSenha(float w, float h) {
        ...
        font.draw(batch, "FB=Folha de Bordo  T=Totem  C=Castor", w/2 - 290, h/2 + 70);
        font.draw(batch, "I=Iglu  XB=Xarope de Bordo  H=Hoquei", w/2 - 270, h/2 + 35);
        batch.end();
        ...
        if (Gdx.input.isKeyJustPressed(Keys.ENTER)) {
            if (senhaDigitada.equals(senhaCorreta)) {
                jogo.setScreen(new TelaVilagemCanada(jogo, jogador, true, false));
            } else {
                senhaErrada = true;
                senhaDigitada = "";
            }
