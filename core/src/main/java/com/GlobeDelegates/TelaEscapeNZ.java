...
public class TelaEscapeNZ implements Screen {
    ...
    private String[] nomesArquivo = {"kiwi", "maori", "ovelha", "rugby", "tuatara", "feto"};
    private String[] nomesCorretos = {"Kiwi", "Guerreiro Maori", "Ovelha", "Rugby", "Tuatara", "Feto Prateado"};
    private String[] iniciais = {"K", "M", "O", "R", "T", "F"};
    ...
    public TelaEscapeNZ(GlobeDelegates jogo, Jogador jogador) {
        this.jogo = jogo;
        ...
        fundo = new Texture("nz/escape.jpg");

        objetos = new Texture[nomesArquivo.length];
        for (int i = 0; i < nomesArquivo.length; i++) {
            objetos[i] = new Texture("nz/" + nomesArquivo[i] + ".png");
        }
    ...
    private void renderSenha(float w, float h) {
        ...
        font.draw(batch, "K=Kiwi  M=Maori  O=Ovelha  R=Rugby", w/2 - 270, h/2 + 70);
        font.draw(batch, "T=Tuatara  F=Feto Prateado", w/2 - 200, h/2 + 35);
        batch.end();
        ...
        if (Gdx.input.isKeyJustPressed(Keys.ENTER)) {
            if (senhaDigitada.equals(senhaCorreta)) {
                jogo.setScreen(new TelaVilagemNZ(jogo, jogador, true, false));
            } else {
                senhaErrada = true;
                senhaDigitada = "";
            }
