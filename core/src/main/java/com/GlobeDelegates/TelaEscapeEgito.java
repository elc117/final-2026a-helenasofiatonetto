...
public class TelaEscapeEgito implements Screen {
    ...
    private String[] nomesArquivo = {"Escaravelho", "cruz", "esfinge", "gato", "mumia", "olho", "papiro", "piramide"};
    private String[] nomesCorretos = {"Escaravelho", "Ankh", "Esfinge", "Gato Sagrado", "Mumia", "Olho de Horus", "Papiro", "Piramide"};
    private String[] iniciais = {"E", "A", "ES", "GS", "M", "OH", "P", "PI"};
    ...
    private float objSize = 90;
    private float btnW = 280, btnH = 50;
    private String[] opcoesMisturadas;
    private float[] objX = {80, 220, 380, 540, 700, 860, 150, 600};
    private float[] objY;

    public TelaEscapeEgito(GlobeDelegates jogo, Jogador jogador) {
        this.jogo = jogo;
        ...
        fundo = new Texture("egito/escape.jpg");

        objetos = new Texture[nomesArquivo.length];
        for (int i = 0; i < nomesArquivo.length; i++) {
            objetos[i] = new Texture("egito/" + nomesArquivo[i] + ".png");
        }

        float h = Gdx.graphics.getHeight();
        objY = new float[]{h*0.55f, h*0.35f, h*0.6f, h*0.4f, h*0.25f, h*0.5f, h*0.3f, h*0.45f};
    ...
    public void render(float delta) {
        Gdx.gl.glClearColor(0.9f, 0.85f, 0.7f, 1);
    ...
    private void renderSenha(float w, float h) {
        ...
        shape.rect(w/2 - 380, h/2 - 220, 760, 440);
        shape.end();
        ...
        font.draw(batch, "E=Escaravelho  A=Ankh  ES=Esfinge  GS=Gato Sagrado", w/2 - 340, h/2 + 90);
        font.draw(batch, "M=Mumia  OH=Olho de Horus  P=Papiro  PI=Piramide", w/2 - 320, h/2 + 55);
        batch.end();
        ...
        if (Gdx.input.isKeyJustPressed(Keys.ENTER)) {
            if (senhaDigitada.equals(senhaCorreta)) {
                jogo.setScreen(new TelaVilagemEgito(jogo, jogador, true, false));
            } else {
                senhaErrada = true;
                senhaDigitada = "";
            }
