...
public class TelaVilagemMexico implements Screen {
    ...
    private float[] casinhaX = {150, 450, 700};
    private float[] casinhaY;
    private float casinhaSize = 80;
    private String[] labels = {"Escape Room", "Delegacao", "Bonus"};
    private boolean[] desbloqueado;

    public TelaVilagemMexico(GlobeDelegates jogo, Jogador jogador, boolean escapeCompleto, boolean delegacaoCompleta) {
        this.jogo = jogo;
        ...
        fundo = new Texture("mexico/telaEscolha.jpg");

        float h = Gdx.graphics.getHeight();
        casinhaY = new float[]{h * 0.45f, h * 0.45f, h * 0.45f};
        ...
        for (int i = 0; i < 3; i++) {
            ...
            if (dist < 80 && desbloqueado[i]) {
                ...
                if (Gdx.input.isKeyJustPressed(Keys.SPACE)) {
                    if (i == 0) jogo.setScreen(new TelaEscapeMexico(jogo, jogador));
                    if (i == 1) jogo.setScreen(new TelaDelegacaoMexico(jogo, jogador));
                    if (i == 2) jogo.setScreen(new TelaBonusMexico(jogo, jogador));
                }
            }
        }
