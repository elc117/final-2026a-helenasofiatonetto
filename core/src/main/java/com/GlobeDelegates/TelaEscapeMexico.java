...
public class TelaEscapeMexico implements Screen {
    ...
    private String[] nomesArquivo = {"pinata", "agave", "maracas", "piramide", "caveira", "sombrero", "flor"};
    private String[] nomesCorretos = {"Pinata", "Agave", "Maracas", "Piramide", "Caveira", "Sombrero", "Flor de Cempasuchil"};
    private String[] iniciais = {"P", "A", "M", "PI", "C", "S", "FC"};
    ...
    private float[] objX = {120, 450, 680, 900, 200, 750, 350};
    private float[] objY;

    public TelaEscapeMexico(GlobeDelegates jogo, Jogador jogador) {
        this.jogo = jogo;
        ...
        fundo = new Texture("mexico/escape.jpg");

        objetos = new Texture[nomesArquivo.length];
        for (int i = 0; i < nomesArquivo.length; i++) {
            objetos[i] = new Texture("mexico/" + nomesArquivo[i] + ".png");
        }
    ...
    public void render(float delta) {
        Gdx.gl.glClearColor(0.9f, 0.85f, 0.8f, 1);
    ...
    private void renderSenha(float w, float h) {
        ...
        font.draw(batch, "P=Pinata  A=Agave  M=Maracas  PI=Piramide", w/2 - 310, h/2 + 70);
        font.draw(batch, "C=Caveira  S=Sombrero  FC=Flor de Cempasuchil", w/2 - 330, h/2 + 35);
        batch.end();
        ...
        if (Gdx.input.isKeyJustPressed(Keys.ENTER)) {
            if (senhaDigitada.equals(senhaCorreta)) {
                jogo.setScreen(new TelaVilagemMexico(jogo, jogador, true, false));
            } else {
                senhaErrada = true;
                senhaDigitada = "";
            }
