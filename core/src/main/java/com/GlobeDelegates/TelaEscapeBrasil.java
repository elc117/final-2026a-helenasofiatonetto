...
public class TelaEscapeBrasil implements Screen {
    ...
    private String[] nomesArquivo = {"arara", "caramelo", "cristo", "filtroBarro", "orelhao", "zegotinha", "feira"};
    private String[] nomesCorretos = {"Arara", "Cachorro Caramelo", "Cristo Redentor", "Filtro de Barro", "Orelhao", "Ze Gotinha", "Pastel e Caldo de Cana"};
    private String[] iniciais = {"A", "CC", "CR", "FB", "O", "ZG", "PC"};
    private String[] extensoes = {".jpg", ".jpg", ".jpg", ".jpg", ".jpg", ".jpg", ".jpg"};
    ...
    private float btnW = 320, btnH = 50;
    private String[] opcoesMisturadas;
    private float[] objX = {100, 280, 460, 640, 820, 180, 580};
    private float[] objY;

    public TelaEscapeBrasil(GlobeDelegates jogo, Jogador jogador) {
        this.jogo = jogo;
        ...
        fundo = new Texture("brasil/escape.jpg");

        objetos = new Texture[nomesArquivo.length];
        for (int i = 0; i < nomesArquivo.length; i++) {
            // orelhão tem caractere especial
            String arquivo = nomesArquivo[i].equals("orelhao") ? "orelhão" : nomesArquivo[i];
            objetos[i] = new Texture("brasil/" + arquivo + extensoes[i]);
        }

        float h = Gdx.graphics.getHeight();
        objY = new float[]{h*0.55f, h*0.35f, h*0.6f, h*0.4f, h*0.25f, h*0.5f, h*0.3f};
    ...
    public void render(float delta) {
        ...
        shape.begin(ShapeRenderer.ShapeType.Filled);
        shape.setColor(0.1f, 0.4f, 0.1f, 0.85f);
        ...
        if (!mostrándoPergunta) {
            ...
            for (int i = 0; i < 4; i++) {
                ...
                if (opcaoSelecionada == i) {
                    ...
                    shape.setColor(0.1f, 0.4f, 0.1f, 1);
                }
    ...
    private void renderSenha(float w, float h) {
        ...
        font.draw(batch, "A=Arara  CC=Cachorro Caramelo  CR=Cristo Redentor", w/2 - 340, h/2 + 90);
        font.draw(batch, "FB=Filtro de Barro  O=Orelhao  ZG=Ze Gotinha  PC=Pastel", w/2 - 360, h/2 + 50);
        batch.end();

        shape.begin(ShapeRenderer.ShapeType.Filled);
        shape.setColor(0.2f, 0.2f, 0.35f, 1);
        shape.rect(w/2 - 200, h/2 - 10, 400, 55);
        shape.end();

        batch.begin();
        font.setColor(0.5f, 1f, 0.5f, 1);
        font.draw(batch, senhaDigitada + "|", w/2 - 185, h/2 + 35);
        if (senhaErrada) {
            font.setColor(1, 0.3f, 0.3f, 1);
            font.draw(batch, "Senha incorreta! Tente novamente.", w/2 - 250, h/2 - 50);
        }
        font.setColor(0.8f, 0.8f, 0.5f, 1);
        font.draw(batch, "ENTER confirmar  |  BACKSPACE apagar", w/2 - 280, h/2 - 100);
        batch.end();
        ...
        if (Gdx.input.isKeyJustPressed(Keys.ENTER)) {
            if (senhaDigitada.equals(senhaCorreta)) {
                jogo.setScreen(new TelaVilagemBrasil(jogo, jogador, true, false));
            } else {
                senhaErrada = true;
                senhaDigitada = "";
            }
