...
public class TelaEscapeAustria implements Screen {
    ...
    private String[] nomesArquivo = {"calca", "chapeu", "flor", "pretzel", "ski", "violino"};
    private String[] nomesCorretos = {"Lederhosen", "Chapeu Tiroles", "Edelweiss", "Pretzel", "Esqui", "Violino"};
    private String[] iniciais = {"L", "CT", "E", "P", "S", "V"};
    ...
    private float[] objX = {120, 350, 580, 810, 200, 650};
    private float[] objY;

    public TelaEscapeAustria(GlobeDelegates jogo, Jogador jogador) {
        this.jogo = jogo;
        ...
        fundo = new Texture("austria/escape.jpg");

        objetos = new Texture[nomesArquivo.length];
        for (int i = 0; i < nomesArquivo.length; i++) {
            objetos[i] = new Texture("austria/" + nomesArquivo[i] + ".png");
        }

        float h = Gdx.graphics.getHeight();
        objY = new float[]{h*0.55f, h*0.35f, h*0.6f, h*0.4f, h*0.25f, h*0.5f};
    ...
    private void renderSenha(float w, float h) {
        ...
        font.draw(batch, "L=Lederhosen  CT=Chapeu Tiroles  E=Edelweiss", w/2 - 300, h/2 + 70);
        font.draw(batch, "P=Pretzel  S=Esqui  V=Violino", w/2 - 200, h/2 + 35);
        batch.end();
        ...
        if (Gdx.input.isKeyJustPressed(Keys.ENTER)) {
            if (senhaDigitada.equals(senhaCorreta)) {
                jogo.setScreen(new TelaVilagemAustria(jogo, jogador, true, false));
            } else {
                senhaErrada = true;
                senhaDigitada = "";
            }
