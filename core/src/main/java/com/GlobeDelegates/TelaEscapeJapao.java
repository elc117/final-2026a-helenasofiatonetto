...
public class TelaEscapeJapao implements Screen {
    ...
    private String[] nomesArquivo = {"koi", "dango", "onigiri", "leque", "torii", "moeda", "cerejeira"};
    private String[] nomesCorretos = {"Carpa Koi", "Dango", "Onigiri", "Leque", "Torii", "Moeda da Sorte", "Cerejeira"};
    private String[] iniciais = {"CK", "D", "O", "L", "T", "MS", "C"};
    ...
    public TelaEscapeJapao(GlobeDelegates jogo, Jogador jogador) {
        this.jogo = jogo;
        ...
        fundo = new Texture("japao/hall.png");

        objetos = new Texture[nomesArquivo.length];
        for (int i = 0; i < nomesArquivo.length; i++) {
            objetos[i] = new Texture("japao/" + nomesArquivo[i] + ".png");
        }
    ...
    private void renderSenha(float w, float h) {
        ...
        font.draw(batch, "CK=Carpa Koi  D=Dango  O=Onigiri  L=Leque", w/2 - 310, h/2 + 70);
        font.draw(batch, "T=Torii  MS=Moeda da Sorte  C=Cerejeira", w/2 - 290, h/2 + 35);
        batch.end();
        ...
        if (Gdx.input.isKeyJustPressed(Keys.ENTER)) {
            if (senhaDigitada.equals(senhaCorreta)) {
                jogo.setScreen(new TelaVilagemJapao(jogo, jogador, true, false));
            } else {
                senhaErrada = true;
                senhaDigitada = "";
            }
